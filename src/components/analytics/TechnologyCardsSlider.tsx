...
function GroupSVG({ svgPaths }: { svgPaths: any }) {
  return (
    <div className="relative w-[316px] h-[268px]">
      ...
      <img alt="" className="absolute inset-0 max-w-none object-50%-50% object-contain pointer-events-none size-full" src={svgPaths} />
    </div>
  );
}
...
export default function TechnologyCardsSlider() {
  ...
  return (
    <div
      ref={containerRef}
      className={`block ${isLargeScreen ? 'min-h-[766px] lg:min-h-[680px] xl:min-h-[766px]' : 'min-h-auto'} overflow-clip relative shrink-0 w-full transition-all duration-500`}
      data-name="Cards"
      style={{ background: 'linear-gradient(242.47deg, #00BCEF 6.45%, #002363 95.13%)' }}
    >
      {/* Header Content */}
      <div className={`${isLargeScreen ? 'absolute' : 'relative'} content-stretch flex ${isLargeScreen ? 'gap-[250px] lg:gap-[120px] xl:gap-[250px]' : 'flex-col gap-[16px]'} ${isLargeScreen ? 'h-[90px] lg:h-[80px] xl:h-[90px]' : 'h-auto'} items-${isLargeScreen ? 'center' : 'start'} ${isLargeScreen ? 'left-[8%] lg:left-[6%] xl:left-[180px]' : 'px-[5%]'} not-italic text-white ${isLargeScreen ? 'top-[70px] lg:top-[50px] xl:top-[70px]' : 'pt-[50px]'} ${isLargeScreen ? 'w-[84%] lg:w-[88%] xl:w-[1152px]' : 'w-full'} opacity-0 animate-[headerSlideIn_0.8s_ease-out_0.2s_forwards]`}>
        <p className={`font-['Gilroy:Semibold',sans-serif] ${isLargeScreen ? 'leading-[45px] lg:leading-[38px] xl:leading-[45px]' : 'leading-[32px]'} relative ${isLargeScreen ? 'shrink-0' : ''} ${isLargeScreen ? 'text-[36px] lg:text-[28px] xl:text-[36px]' : 'text-[28px]'} ${isLargeScreen ? 'w-[260px] lg:w-[220px] xl:w-[260px]' : 'w-full'}`}>{`Technology & Integrations`}</p>
        <p className={`${isLargeScreen ? 'basis-0' : ''} font-['Gilroy:Medium',sans-serif] ${isLargeScreen ? 'grow' : ''} ${isLargeScreen ? 'leading-[24px] lg:leading-[20px] xl:leading-[24px]' : 'leading-[20px]'} ${isLargeScreen ? 'min-h-px min-w-px' : ''} relative ${isLargeScreen ? 'shrink-0' : ''} ${isLargeScreen ? 'text-[16px] lg:text-[14px] xl:text-[16px]' : 'text-[14px]'}`}>Built on a robust, scalable, and secure architecture - Tecosoft Analytics seamlessly integrates with your enterprise ecosystem, combining IoT connectivity, AI analytics, and open APIs to deliver intelligent insights instantly.</p>
      </div>
      ...
      {isLargeScreen ? (
              ...
              >
                <div className="flex flex-row items-center justify-center size-full">
                  <div className="box-border content-stretch flex gap-[10px] items-center justify-center px-[24px] lg:px-[18px] xl:px-[24px] py-[10px] lg:py-[8px] xl:py-[10px] relative w-full">
                    <p className={`font-['Gilroy:${activeIndex === index ? 'Semibold' : 'Medium'}',sans-serif] leading-[28px] lg:leading-[24px] xl:leading-[28px] not-italic relative shrink-0 text-[18px] lg:text-[15px] xl:text-[18px] text-nowrap whitespace-pre ${
        ...
        <>
          ...
          <div className="relative w-full px-[5%] py-[40px] pb-[60px] flex flex-col gap-[24px] opacity-0 animate-[cardFadeIn_1s_ease-out_0.6s_forwards]">
            {cardData.map((card, index) => (
              <div key={card.id} className="bg-white relative rounded-[20px] w-full transition-all duration-300 hover:shadow-2xl p-[30px] pb-[40px]">
                <div className="absolute inset-0 pointer-events-none shadow-[4px_4px_12px_0px_inset_rgba(0,0,0,0.12)] rounded-[20px]" />

                {/* Card Content */}
                <div className="relative flex flex-col gap-[16px]">
                  <p className="font-['Gilroy:Semibold',sans-serif] leading-[28px] not-italic text-[#0098d4] text-[24px]">
                    {card.title}
                  </p>
                  ...
                  <div className="flex justify-center my-[20px]">
                    <div className="w-[250px] h-[212px]">
                      <img alt="" className="w-full h-full object-contain" src={card.svgPaths} />
                    </div>
                  </div>

                  <p className="font-['Gilroy:Regular',sans-serif] leading-[22px] not-italic text-[#636363] text-[16px]">
                    {card.description}
                  </p>
                  ...
                  <div className="flex gap-[40px] mt-[16px]">
                    <div className="flex flex-col gap-[4px]">
                      <p className="font-['Gilroy:Semibold',sans-serif] leading-[40px] text-[#282828] text-[38px]">
                        {card.metric1.value}
                      </p>
                      <p className="font-['Gilroy:Regular',sans-serif] leading-[18px] text-[#636363] text-[14px]">
                        {card.metric1.label}
                      </p>
                    </div>
